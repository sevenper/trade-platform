...
import { tablePage, newStockColumns, dialogConfig } from "./config";
...
const buyApi = async () => {
    let params = {
        ...
        // buyNum: this.haoForm.shehao,
        // passWord:this.haoForm.pss
    };
...
const payMoney = computed(
  () => (buyNum.value * itemData.price).toFixed(2) || 0
);
...
<template>
  ...
  <div class="content">
    <a-tabs
      ...
      @change="countryClick(activeKey)"
    >
      <a-tab-pane
        :tab="item.label"
        :key="item.key"
        v-for="item in stockTypeV"
      ></a-tab-pane>
    </a-tabs>
    <div class="table-div">
      <tableList
        :columnsData="newStockColumns"
        :dataLists="dataLists"
        :tablePage="tablePage"
        @getList="getList"
